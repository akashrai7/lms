<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Nationality" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Nationality' : 'Add New Nationality' }}</h5>
...
const fetchNationality = async () => {
  loading.value = true;
  try {
    const res = await $fetch('/api/nationality', { credentials: 'include' });
...
onMounted(() => fetchNationality());

/* ------------ Search + Pagination ------------ */
const filteredItems = computed(() =>
  nationality.value.filter(item =>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/nationality/${form.value.id}`, {
        method: "PUT",
        body: { name: form.value.name },
        credentials: "include",
      });
       showAlert("Nationality updated successfully!", "success");
    } else {
      await $fetch("/api/nationality/add", {
        method: "POST",
        body: { name: form.value.name },
        credentials: "include",
        headers: { Authorization: `Bearer ${token}` },
      });
      showAlert("Nationality added successfully!", "success");
    }
    resetForm();
    fetchNationality();
  } catch (error) {
    console.error("Save Error:", error);
    showAlert("Failed to save Nationality", "danger");
  } finally {
    loading.value = false;
  }
};
...
/* ------------ Edit ------------ */
const editInstitute = (item: Nationality) => {
  isEdit.value = true;
...
const deleteInstitute = async (id: string) => {
  ...
  loading.value = true;
  try {
    await $fetch(`/api/nationality/${id}`, {
      method: "DELETE",
      credentials: "include",
      headers: { Authorization: `Bearer ${token}` },
    });

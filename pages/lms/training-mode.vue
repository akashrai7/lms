<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Training Mode" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Training Mode' : 'Add New Training Mode' }}</h5>
          ...
          <form @submit.prevent="handleSubmit">
            <!-- Name Field -->
            <div class="mb-3">
              <label class="form-label">Training Mode Name</label>
              <input
                v-model="form.name"
                type="text"
                class="form-control"
                placeholder="Enter Training Mode name"
...
import type { TrainingMode } from '~/types/training-mode';

const trainingMode = ref<TrainingMode[]>([]);
...
const fetchTrainingMode = async () => {
  loading.value = true;
  try {
    const res = await $fetch('/api/training-mode', { credentials: 'include' });
    trainingMode.value = res.data || [];
  } catch (error) {
...
onMounted(() => fetchTrainingMode());

/* ------------ Search + Pagination ------------ */
const filteredItems = computed(() =>
  trainingMode.value.filter(item =>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/training-mode/${form.value.id}`, {
        method: "PUT",
        body: { name: form.value.name },
        credentials: "include",
      });
       showAlert("Training Mode updated successfully!", "success");
    } else {
      await $fetch("/api/training-mode/add", {
        method: "POST",
        body: { name: form.value.name },
        credentials: "include",
        headers: { Authorization: `Bearer ${token}` },
      });
      showAlert("Training Mode added successfully!", "success");
    }
    resetForm();
    fetchTrainingMode();
  } catch (error) {
    console.error("Save Error:", error);
    showAlert("Failed to save Training Mode", "danger");
  } finally {
    loading.value = false;
  }
};
...
/* ------------ Edit ------------ */
const editInstitute = (item: TrainingMode) => {
  isEdit.value = true;
...
const deleteInstitute = async (id: string) => {
  ...
  loading.value = true;
  try {
    await $fetch(`/api/training-mode/${id}`, {
      method: "DELETE",
      credentials: "include",
      headers: { Authorization: `Bearer ${token}` },
    });

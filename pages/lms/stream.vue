<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Stream" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Stream' : 'Add New Stream' }}</h5>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/stream/${form.value.id}`, {
        method: "PUT",
      ...
      showAlert("Stream added successfully!", "success");
    }
    resetForm();
    fetchStream();
  } catch (error) {
    console.error("Save Error:", error);
    showAlert("Failed to save Stream", "danger");
  } finally {
    loading.value = false;
  }
};
...
/* ------------ Edit ------------ */
const editInstitute = (item: Stream) => {
  isEdit.value = true;

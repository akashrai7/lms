<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Language" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Language' : 'Add New Language' }}</h5>
...
const fetchLanguage = async () => {
  loading.value = true;
  try {
    const res = await $fetch('/api/language', { credentials: 'include' });
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/language/${form.value.id}`, {
        method: "PUT",
    ...
    showAlert("Failed to save Language", "danger");
  } finally {
    loading.value = false;
  }
};
...
/* ------------ Edit ------------ */
const editInstitute = (item: Language) => {
  isEdit.value = true;

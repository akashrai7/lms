<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Section" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Section' : 'Add New Section' }}</h5>
...
const fetchSection = async () => {
  loading.value = true;
  try {
    const res = await $fetch('/api/section', { credentials: 'include' });
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/section/${form.value.id}`, {
        method: "PUT",
...
/* ------------ Edit ------------ */
const editInstitute = (item: Section) => {
  isEdit.value = true;

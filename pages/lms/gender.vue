<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Gender" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Gender' : 'Add New Gender' }}</h5>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/gender/${form.value.id}`, {
        method: "PUT",
        body: { name: form.value.name },
        credentials: "include",
      });
       showAlert("Gender updated successfully!", "success");
    } else {
      await $fetch("/api/gender/add", {
        method: "POST",
        body: { name: form.value.name },
        credentials: "include",
    ...
    showAlert("Failed to save Gender", "danger");
  } finally {
    loading.value = false;
  }
};
...
/* ------------ Edit ------------ */
const editInstitute = (item: Gender) => {
  isEdit.value = true;

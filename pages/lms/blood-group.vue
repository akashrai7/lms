<template>
 <div class="main-content-container overflow-hidden">
    <CommonPageTitle pageTitle="Blood Group" subTitle="LMS" />
  <div class="row justify-content-center">
    <!-- Form Section -->
    <div class="col-xxl-4 col-md-4">
      <div class="card bg-white border-0 rounded-3 mb-4">
        <div class="card-body p-4">
          <h5 class="mb-3 border-bottom pb-2">{{ isEdit ? 'Edit Blood Group' : 'Add New Blood Group' }}</h5>
...
import type { BloodGroup } from '~/types/bloodGroup';

const bloodGroup = ref<BloodGroup[]>([]);
...
const fetchBloodGroup = async () => {
  loading.value = true;
  try {
    const res = await $fetch('/api/blood-group', { credentials: 'include' });
    bloodGroup.value = res.data || [];
  } catch (error) {
...
onMounted(() => fetchBloodGroup());

/* ------------ Search + Pagination ------------ */
const filteredItems = computed(() =>
  bloodGroup.value.filter(item =>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEdit.value && form.value.id) {
      await $fetch(`/api/blood-group/${form.value.id}`, {
        method: "PUT",
        body: { name: form.value.name },
        credentials: "include",
      });
       showAlert("Blood Group updated successfully!", "success");
    } else {
      await $fetch("/api/blood-group/add", {
        method: "POST",
        body: { name: form.value.name },
        credentials: "include",
        headers: { Authorization: `Bearer ${token}` },
      });
      showAlert("Blood Group added successfully!", "success");
    }
    resetForm();
    fetchBloodGroup();
  } catch (error) {
    console.error("Save Error:", error);
    showAlert("Failed to save Blood Group", "danger");
  } finally {
    loading.value = false;
  }
};
...
/* ------------ Edit ------------ */
const editInstitute = (item: BloodGroup) => {
  isEdit.value = true;
...
const deleteInstitute = async (id: string) => {
  ...
  loading.value = true;
  try {
    await $fetch(`/api/blood-group/${id}`, {
      method: "DELETE",
      credentials: "include",
      headers: { Authorization: `Bearer ${token}` },
    });
